...
class SubjectFeeView extends StatefulWidget {
  const SubjectFeeView({super.key});

  @override
  State<StatefulWidget> createState() => _SubjectFeeView();
}

class _SubjectFeeView extends State<SubjectFeeView> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> doSomething() async {
    ...
    await accountSession.fetchSubjectFee();
  }
                      ...
                      Expanded(
                        child: Text(
                          StringUtils.formatString(
                            ...
                            [
                              accountSession.subjectFeeList.lastRequest == 0
                                  ? AppLocalizations.of(context).translate("data_unknown")
                                  : DateFormat("dd/MM/yyyy HH:mm").format(
                                      DateTime.fromMillisecondsSinceEpoch(
                                        accountSession.subjectFeeList.lastRequest,
                                      ),
                                    ),
                            ],
                          ),
